...
//计算两个类之间相似性的时候，从 YJL_LSI文件拷贝过来的
public class YJL_LSI_ClassVsClass implements IRModel {
    ...
    public SimilarityMatrix Compute(ArtifactsCollection source, ArtifactsCollection target) {
        ...
        //这里得修改，下面的不是直接调用的
        return ComputeClassVsClass(new TermDocumentMatrix(source), new TermDocumentMatrix(target),
     ...
     * wfpan 修改后的
     * YJL 求两个类之间相似性的时候该的 原名 Compute2，现名 ComputeClassVsClass
     * */
    private SimilarityMatrix ComputeClassVsClass(TermDocumentMatrix source, TermDocumentMatrix target, TermDocumentMatrix both) {
    	System.out.println("[Info] Compute ClassVsClass similarity...");
//        TermDocumentMatrix TF = ComputeTF(source);
//        double[] IDF = ComputeIDF(ComputeDF(source), source.NumDocs());
    	
    	TermDocumentMatrix TF = ComputeTF(both);
//    	System.out.println("term ->"  + TF.getTermName(0));
    ...
    private TermDocumentMatrix svd(TermDocumentMatrix tfidf_origin) {
        ...
        //----新加的 2025-07-18-------//
//      System.out.println(realMatrix.getColumnDimension() + "->" + realMatrix.getRowDimension() + "->" + LSI_K);
        if(LSI_K>realMatrix.getColumnDimension()) {
        	LSI_K = realMatrix.getColumnDimension();
        }
        //----------//
...
//      System.out.println(Arrays.toString(df)+"===="+numDocs);
        for (int i = 0; i < df.length; i++) {
        	if(Double.compare(df[i], 0.0d)<=0) {
        		idf[i] = 0.0d;
        	} else {
//        		System.out.print("here"+numDocs / df[i]);
        		
//        		System.exit(0);
                idf[i] = Math.log((numDocs+1) / df[i]); //为了让相同的文档结果不为0，这里可以让 numDocs+1。原来的版本这里没有+1
            }
...
//        System.out.println(numDocs + " idf->" + Arrays.toString(idf) );
        return idf;
    }
...
//        System.out.println("df -> " + Arrays.toString(df));
    ...
    private SimilarityMatrix ComputeSimilarities(TermDocumentMatrix ids, TermDocumentMatrix tfidf) {
    	System.out.println("[INFO] Compute Similarities...");
...
//        System.out.println(ids.NumDocs() + " -> " + tfidf.NumDocs() + "->->" + matrices.get(0).NumTerms());
...
//                System.out.println(ids.getDocumentName(i) + "-->>" + tfidf.getDocumentName(j) +  "-->>" + product / cross);
    ...
    public static void initProjectKvalue() {
        ...
        projectKvalueMap.put("test", 16); //原来是70
        ...
        projectKvalueMap.put("ant_main", 70); //修改3
        projectKvalueMap.put("argouml", 70);
        projectKvalueMap.put("gwtportlets", 70);
        projectKvalueMap.put("javaclient", 70);
        projectKvalueMap.put("jedit", 70);
        projectKvalueMap.put("jgap", 70);
        projectKvalueMap.put("jhotdraw", 70);
        projectKvalueMap.put("jmeter_core", 70);
        projectKvalueMap.put("JPMC", 70);
        projectKvalueMap.put("log4j", 70);
        projectKvalueMap.put("Maze", 70);
        projectKvalueMap.put("Mars", 70);
        projectKvalueMap.put("neuroph", 70);
        projectKvalueMap.put("PDFBox", 70);
        projectKvalueMap.put("tomcat", 70);
        projectKvalueMap.put("wro4j", 70);
        projectKvalueMap.put("Xerces", 70);
        projectKvalueMap.put("xuml", 70);
    }
	...
	public String toString() {
		return "YJL_LSI_ClassVsClass [queries=" + queries + ", documents=" + documents + ", LSI_K=" + LSI_K + "]";
	}
