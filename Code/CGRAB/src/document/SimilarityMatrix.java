...
public class SimilarityMatrix implements Serializable, Cloneable {
    ...
    /**保存多条连接关系的 Map&lt;sourceArtifactId (ex:req),&lt;targetArtifactId (ex:class),score&gt;&gt;*/
...
//            System.out.println(link.getTargetArtifactId());
    ...
    /**增加连接关系 Map&lt;sourceArtifactId (ex:req),&lt;targetArtifactId (ex:class),score&gt;&gt;*/
    ...
    public boolean isLinkAboveThreshold(String sourceArtifactId, String targetArtifactId) {
        ...
        if (links != null) {
            ...
            if (score != null) {
            	//因为存储的target的都是rtm中真实的连接关系，没有连接的没保存，所以出来的值都为1（默认值），肯定是 >=0的，也不会存在等于0的情况（下面的比较不对，但是不影响结果）- wfpan 2023-07-02
