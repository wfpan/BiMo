...
public class TermDocumentMatrix implements Serializable {
	...
	/**docIndex*termIndex:存储的是 term 的个数*/
    private double[][] matrix;
    /**term name might denote the word in req and class (ex: the words in req1.txt or AbstractASTTransformation.txt)*/
    private List<String> termNameList;
    /**docName might denote the name (ex: req1.txt -> req1 or Student.java -> Student) of req.txt or class .java file*/
    private List<String> docNameList;
    /**termName->order:term在termNameList中的下标*/
    private Map<String, Integer> termNameOrderMap;
    /**docName->order:doc在docNameList中的下标*/
    private Map<String, Integer> docNameOrderMap;

    public TermDocumentMatrix() {
    }

    /**取得某个 doc 中 某个 term 出现的次数*/
    public double getValue(int docIndex, int termIndex) {
        return getMatrix()[docIndex][termIndex];
    }

    /**设置某个 doc 中 某个 term 出现的次数*/
    public void setValue(int docIndex, int termIndex, double value) {
        getMatrix()[docIndex][termIndex] = value;
    }

    /**取得 docIndex*termIndex matrix*/
    public double[][] getMatrix() {
        return matrix;
    }

    /**设置 docIndex*termIndex matrix*/
    ...
    /**返回termNameList的大小，也就是 term 的数量 req中term 的数量 or classes 中term 的数量 or 两者的合集中的数量*/
    public int NumTerms() {
        return termNameList.size();
    }

    /**返回docNameList的大小，也就是 doc 的数量 req的数量 or classes 的数量*/
    public int NumDocs() {
        return docNameList.size();
    }

    /**构建 term * document 矩阵：docNameList、docNameOrderMap、termNameList、termNameOrderMap、matrix*/
    public TermDocumentMatrix(ArtifactsCollection artifacts) {
        ...
        // allDocTermMap -> Map<id 文件名,Map<term 单词,count 次数>>
